...
class Post extends Model
{
    ...
    public function post_tags(){
        // dd($this->hasMany(PostTagTags::class, 'post_id', 'id'));
        return $this->hasMany(PostTagTags::class);
    ...
    public static function getAllPost(){
        if(auth()->user()):
            return Post::where('store_id',auth()->user()->store_id)->with(['cat_info','author_info'])->orderBy('id','DESC')->paginate(10);
    ...
    public static function countActivePost(){
        $data=Post::where('store_id',auth()->user()->store_id)->where('status','active')->count();
        if($data){
            return $data;
        }
        return 0;
