<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      {{-- @if(session('cart'))
      <li class="nav-item dropdown no-arrow mx-1">
        <a class="nav-link dropdown-toggle" href="{{route('shopOrder')}}}}" target="_blank" data-toggle="tooltip" data-placement="bottom" title="home"  role="button">
    ...
    @include('backend.layouts.cart_icon_html')
    ...
    @if(Auth::user()->role=='admin')
      ...
      @php
              $user=DB::table('users')->where('id','photor')->select('users.photo')->get();  
      ...
      <li class="nav-item dropdown no-arrow">
        <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <span class="mr-2 d-none d-lg-inline text-gray-600 small">{{Auth()->user()->name}}</span>
           ...
           @if(Auth()->user()->photo)
            <img class="img-profile rounded-circle" src="{{ asset('images/profile/'.Auth()->user()->photo) }}" >
          @else
