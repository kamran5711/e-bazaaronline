...
@section('main-content')
                    ...
                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                        ...
                        <div class="carousel-caption d-none d-md-block text-left">
                            ...
                            <a class="btn btn-lg ws-btn wow fadeInUpBig slide_button"
                                href="{{ route('product-grids') }}?k={{ request()->get('k') }}" role="button">Shop Now<i
                            ...
                            <div class="card-shadow m-3">
                                <form action="{{ route('add-to-cart', $product->id) }}" method="POST">
                                    ...
                                    <div class="single-product">
                                        ...
                                        </div> <input type="hidden" value="{{ $product->id }}" name="product_id" />
                                        <table>
                                            @php
                                                $color_title = $size_title = null;
                                                if($product->productStock->count() > 0){
    ...
    <!-- Start Shop Blog  -->
    <section class="shop-blog section mt-3 mb-5">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="section-title">
            ...
            <div class="row">
                @if ($posts)
                    @foreach ($posts as $post)
                        <div class="col-lg-3 col-md-6 col-12" style="border-radius: 5px;">
                            <!-- Start Single Blog  -->
                            <div class="shop-single-blog border border-light" style="background: #F6F7FB;">
                                <img src="{{ asset('images/posts/' . $post->photo) }}"
                                    alt="{{ $post->photo }}">
                                <div class="content mb-3 mt-3" style="text-align: center;">
                                    <p class="date" style="color:black;">
                                        {{ $post->created_at->format('d M , Y. D') }}</p>
                                    <a href="{{ route('blog.detail', $post->slug) }}?k={{ request()->get('k') }}"
                                        class="title" style="font-size: 14px; color:black;">
                                        {{ Str::limit($post->title, 70, $end = '...') }}</a>
                                    <a href="{{ route('blog.detail', $post->slug) }}?k={{ request()->get('k') }}"
                                        style="color:blue;" class="more-btn text-warning">Continue
                                        Reading</a>
                                </div>
                            </div>
                            <!-- End Single Blog  --------->
